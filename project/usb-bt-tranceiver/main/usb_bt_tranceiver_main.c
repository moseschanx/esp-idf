...
#define EXAMPLE_USB_DEVICE_DUAL_PID (0x5740) // 0x303A:0x4002 (TinyUSB Dual CDC device)
 ...
 *
 * Allow enumeration only for the target CDC device (STM32 VCP).
 * Required when CONFIG_USB_HOST_ENABLE_ENUM_FILTER_CALLBACK=y.
 *
 * @param[in]  dev_desc          Pointer to the device descriptor of the USB device being enumerated
 * @param[out] bConfigurationValue Configuration value to use for this device (set to 1)
 * @return true  to proceed with enumeration
...
{
    if (dev_desc->idVendor == EXAMPLE_USB_DEVICE_VID &&
            (dev_desc->idProduct == EXAMPLE_USB_DEVICE_PID ||
             dev_desc->idProduct == EXAMPLE_USB_DEVICE_DUAL_PID)) {
...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    switch (event) {
    	case ESP_GATTS_REG_EVT:
            ...
            ESP_LOGI(GATTS_TABLE_TAG, "Setting the GAP device name: %s", SAMPLE_DEVICE_NAME);
